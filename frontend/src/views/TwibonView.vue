  ...
  <div class="p-4 md:p-6 min-h-full">
    ...
    <div class="flex flex-col lg:flex-row gap-6 items-start">
      ...
      <div class="w-full lg:w-80 flex-shrink-0 space-y-4">
        ...
        <div class="card p-4">
          ...
          <div v-if="bgType==='color'" class="flex items-center gap-3">
            ...
            <input type="text" v-model="bgColor"
              class="input flex-1 font-mono text-sm uppercase" placeholder="#ffffff" />
          </div>
        ...
        <!-- ASSETS ─────────────────────────────────────────────── -->
        <div class="card p-4">
          <h3 class="font-semibold text-sm text-claude-graphite mb-3">Assets / Gambar</h3>
          ...
          <div class="mb-4">
            ...
            <div v-else class="grid grid-cols-4 gap-1.5 max-h-44 overflow-y-auto pr-0.5">
              <button
                v-for="a in assetLibrary" :key="a.id"
                @click="addLayer(a)"
                :disabled="!a.img"
                :title="a.name + ' — klik untuk tambahkan'"
            ...
            <p class="text-xs text-claude-dust mt-1.5 text-center">
              Klik gambar untuk letakkan di canvas
            </p>
          </div>
          ...
          <div class="border-t border-claude-line pt-3">
            ...
            <div v-else class="space-y-1 mb-2">
              <div
                v-for="(l, i) in [...layers].reverse()" :key="l.id"
              ...
              >
                ...
                <div class="flex items-center gap-0.5 flex-shrink-0">
                  <button @click.stop="moveLayerUp(layers.length - 1 - i)"
                    ...
                    class="p-1 text-claude-dust hover:text-claude-graphite disabled:opacity-30 transition-colors" title="Naik">
                  ...
                  <button @click.stop="moveLayerDown(layers.length - 1 - i)"
                    ...
                    class="p-1 text-claude-dust hover:text-claude-graphite disabled:opacity-30 transition-colors" title="Turun">
                  ...
                  <button @click.stop="removeLayer(layers.length - 1 - i)"
                    class="p-1 text-red-400 hover:text-red-600 transition-colors" title="Hapus">
            ...
            <Transition name="slide-fade">
              <div v-if="selectedLayerIdx >= 0 && layers[selectedLayerIdx]"
                ...
                <SliderRow label="Ukuran (skala)"
                  ...
                  v-model="layers[selectedLayerIdx].scale"
                  min="0.02" max="5" step="0.01" />
                <div class="grid grid-cols-2 gap-2">
                  <SliderRow label="Posisi X"
                    :display="Math.round(layers[selectedLayerIdx].x*100)+'%'"
                    v-model="layers[selectedLayerIdx].x"
                    min="-0.5" max="1.5" step="0.005" />
                  <SliderRow label="Posisi Y"
                    :display="Math.round(layers[selectedLayerIdx].y*100)+'%'"
                    v-model="layers[selectedLayerIdx].y"
                    min="-0.5" max="1.5" step="0.005" />
                </div>
        ...
        <div class="card p-4">
          <div class="flex items-center justify-between mb-3">
            <h3 class="font-semibold text-sm text-claude-graphite">Teks</h3>
      ...
      <!-- ══════════════════════ CANVAS ═════════════════════════ -->
      <div class="flex-1 flex flex-col items-center sticky top-6">
        <p class="text-xs text-claude-dust mb-2">Preview Instagram Story</p>
        <div class="relative rounded-xl overflow-hidden shadow-xl border border-claude-line"
        ...
        <p class="text-xs text-claude-dust mt-2">{{ DISPLAY_W }}×{{ DISPLAY_H }}px → Export 1080×1920px</p>
...
<script setup>
import { ref, reactive, computed, watch, onMounted, nextTick, h, defineComponent } from 'vue'
...
// ── Asset library ──────────────────────────────────────────────────────────
let assetIdSeq = 0
const assetLibrary = ref([])   // [{ id, name, url, img }]

// ── Canvas layers (placed assets) ──────────────────────────────────────────
let layerIdSeq = 0
const layers          = ref([])    // [{ id, name, url, img, x, y, scale }]
...
// ── Drag state ─────────────────────────────────────────────────────────────
const dragMode         = ref(null)   // null|'layer-move'|'layer-resize'|'text'
...
// ── Load default assets from images folder ─────────────────────────────────
...
function getLayerBoxDisplay(layer) {
  return getLayerRect(layer, DISPLAY_W, DISPLAY_H)
}
...
function onMouseDown(e) {
  ...
  // 1 · Check selected layer corner handles first
  ...
  // 2 · Check all layer bodies
  const li = findLayerAt(mx, my)
  if (li >= 0) {
    selectedLayerIdx.value = li
    selectedTextIdx.value  = -1
    dragging.value         = true
    dragMode.value         = 'layer-move'
    dragLayerIdx.value     = li
  ...
  if (ti >= 0) {
    selectedTextIdx.value  = ti
    selectedLayerIdx.value = -1
    dragging.value         = true
    dragMode.value         = 'text'
    dragTextIdx.value      = ti
